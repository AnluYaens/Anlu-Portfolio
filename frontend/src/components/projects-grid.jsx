...
const ProjectCard = ({ project, variant }) => {
  ...
  const imageHeight = variant === "featured" ? "h-56" : "h-44";
  const titleSize = variant === "featured" ? "text-2xl" : "text-xl";
  const padding = variant === "featured" ? "p-6" : "p-5";

  return (
    <article className="group relative bg-white/5 rounded-2xl border border-white/10 hover:border-blue-500/50 hover:bg-white/10 transition-all duration-300 flex flex-col backdrop-blur-sm overflow-hidden h-full">
      <div className="relative w-full overflow-hidden">
        {project.image_url ? (
          <img
            src={project.image_url}
            alt={project.title}
            className={`w-full ${imageHeight} object-cover border-b border-white/5 shadow-lg group-hover:scale-105 transition-transform duration-300`}
          />
        ) : (
          <div
